...
public class Authentication {
	...
	private boolean authenticate(String username, String password) throws SQLException{
		...
		ResultSet resultSet = Database.tableRequest("SELECT COUNT(*) as exist FROM user WHERE username = '" + username + "' AND password = '" + password + "'");
		if(resultSet.next()){
	...
	private String issueToken(String username) throws SQLException, ParseException, NoSuchAlgorithmException, InvalidKeyException, NoSuchPaddingException, IllegalBlockSizeException, BadPaddingException{
		...
		int id = 0;
		String token = "ERROR";
		ResultSet resultSet = Database.tableRequest("SELECT role, id FROM user WHERE username ='" + username + "'");
		if(resultSet.next()){
			...
			Database.operationOnTable("UPDATE user SET token='" + token + "', token_expiration_date = date('" + date + "') WHERE username ='" + username + "'");
		}
